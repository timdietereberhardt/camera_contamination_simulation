...
class CombinedSegmentationDataset(Dataset):
    def __init__(self, image_dirs, mask_dirs, transform=None):
        ...
        # Combine all image names from both directories
        self.image_names = []
        for image_dir in image_dirs:
            self.image_names.extend(os.listdir(image_dir))

    def __len__(self):
        return len(self.image_names)

    def __getitem__(self, idx):
        # Select the correct directory based on the index
        image_dir = self.image_dirs[0] if idx < len(os.listdir(self.image_dirs[0])) else self.image_dirs[1]
        mask_dir = self.mask_dirs[0] if idx < len(os.listdir(self.mask_dirs[0])) else self.mask_dirs[1]
        image_idx = idx if idx < len(os.listdir(self.image_dirs[0])) else idx - len(os.listdir(self.image_dirs[0]))

        # Load the image and mask
        image_path = os.path.join(image_dir, os.listdir(image_dir)[image_idx])
        mask_path = os.path.join(mask_dir, os.listdir(mask_dir)[image_idx])
...
# Directory paths
image_dirs = ['/data/train/rgb_total',
              '/data/train/rgb_semi']
mask_dirs = ['/data/train/gt_total',
             '/data/train/gt_semi']
save_path = '/unet_detection/models/bestmodel_alldata.pth'
...
dataset = CombinedSegmentationDataset(image_dirs=image_dirs, mask_dirs=mask_dirs, transform=transform)
train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

# For simplicity, we will split the dataset into train and validation using the same dataset here
# You may want to create separate validation data
