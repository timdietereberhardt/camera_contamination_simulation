...
# Define the custom Dataset class with split logic for training and validation
class CombinedSegmentationDataset(Dataset):
    def __init__(self, image_dirs, mask_dirs, transform=None, use_aug=False):
        """
        Args:
        ...
        # Only use the original (non-augmented) directories for validation
        if use_aug:
            self.image_dirs = image_dirs
            self.mask_dirs = mask_dirs
        else:
            self.image_dirs = [image_dirs[0], image_dirs[2]]  # Use only non-augmented directories
            self.mask_dirs = [mask_dirs[0], mask_dirs[2]]  # Same for masks

        # Collect all image and mask paths
        for image_dir, mask_dir in zip(self.image_dirs, self.mask_dirs):
            image_files = sorted(os.listdir(image_dir))  # Make sure the files are sorted
...
save_path = '/unet_detection/models/bestmodel_50.pth'
...
train_dataset = CombinedSegmentationDataset(image_dirs=image_dirs, mask_dirs=mask_dirs, transform=transform, use_aug=True)
val_dataset = CombinedSegmentationDataset(image_dirs=image_dirs, mask_dirs=mask_dirs, transform=transform, use_aug=False)
